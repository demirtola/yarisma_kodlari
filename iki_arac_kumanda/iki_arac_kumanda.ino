...
uint8_t broadcastAddressMini[] = {0xD4, 0xD4, 0xDA, 0xFC, 0xD5, 0xE8};
...
esp_now_peer_info_t peerInfoMini;
...
void setup() {
  ...
  memcpy(peerInfoMini.peer_addr, broadcastAddressMini, 6);
  peerInfoMini.channel = 0;
  peerInfoMini.encrypt = false;

  // Add peer
  if (esp_now_add_peer(&peerInfo) != ESP_OK || esp_now_add_peer(&peerInfoMini) != ESP_OK) {
...
void loop() {
  ...
  //esp_err_t result = esp_now_send(broadcastAddressMini, (uint8_t *) &myData, sizeof(myData));
